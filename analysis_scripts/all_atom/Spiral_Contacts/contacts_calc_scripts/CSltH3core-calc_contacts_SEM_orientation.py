...
# Input files
# ------------------------
pdb_file  = "CSltH3core_1.init_for_cafemol.pdb"
traj_file = "total.xtc"

# Output
output_file = "CSltH3core_tail_contacts_inside_outside_freq_SEM.dat"

# ------------------------
# Parameters
# ------------------------
entrytail_residues = '785:827'
exittail_residues  = '295:337'
dna_residues	   = '1:294'
...
entrytail = u.select_atoms(f'resid {entrytail_residues} and not name H*')
exittail  = u.select_atoms(f'resid {exittail_residues} and not name H*')
dna	   = u.select_atoms(f'resid {dna_residues} and not name H*')
histone_core = u.select_atoms("(resid 379-406 or resid 869-896) and protein")
...
for ts in u.trajectory:
	...
	for res in dna.residues[:147]:
		try:
			C1 = res.atoms.select_atoms("name C1'")[0].position
			C3 = res.atoms.select_atoms("name C3'")[0].position
		except IndexError:
			continue
		vec = C3 - C1
		radial = histone_core.center_of_mass() - C1
